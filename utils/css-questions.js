const cssQuestions = [
  ...
  },
  {
    ...
    answer:
      "CSS Modules — це підхід, при якому кожен файл стилів компілюється у локально-ізольовані класи. Замість глобальних назв (.btn, .title) ви отримуєте у збірці унікальні імена з хешами (.btn__3f9a1), тож стилі різних компонентів не конфліктують. Навіщо це Локальна ізоляція: жодних «протікань» стилів між компонентами. Проста ментальна модель: пишете звичайний CSS/SCSS, але без страху колізій. Працює з будь-яким бандлером: Webpack, Vite, Parcel; в багатьох фреймворках увімкнено «з коробки». Композиція класів: можна збирати стилі з кількох класів/файлів.",
  },
  {
    id: 37,
  ...
  {
    id: 40,
    question: "Як можна оптимізувати продуктивність CSS на веб-сторінках?",
    answer:
  ...
  },
  {
    ...
    answer:
      "У SCSS змінні — це імена для значень (кольори, відступи, шрифти тощо), які підставляються ще на етапі компіляції. Пишуться через $",
  },
  {
    id: 43,
  ...
  },
  {
    ...
    answer:
      "@extend у SCSS дає «наслідування стилів»: він підмішує селектор-нащадок у правило предка, щоб обидва (і всі, хто розширює) ділили один набір декларацій.",
  },
  {
    id: 46,
    question:
    ...
    answer:
      "У SCSS partials — це «часткові» файли для розбиття стилів на модулі. Вони: мають ім’я з підкресленням на початку: _variables.scss, _mixins.scss; не компілюються в окремий CSS самі по собі; підключаються в інші файли через модульну систему @use / @forward (рекомендовано) або старий @import (застаріло).",
  },
  {
    id: 47,
    question:
    ...
    answer:
      "Оператори: математика, порівняння, логіка, інтерполяція — для обчислень під час збірки. Функції: модульні інструменти для кольорів/рядків/списків/мап/математики + власні @function.",
  },
  {
    id: 48,
  ...
  },
  {
    ...
    answer:
      "Міксини (@mixin / @include) вставляють копію декларацій у місце виклику. Можуть мати параметри й @content. Гнучкі, але можуть збільшувати вагу CSS (дублювання коду). Плейсхолдери (%name + @extend) об’єднують селектори в одне правило. Код не дублюється, але без параметрів і є нюанси з контекстом (@media, специфічністю).",
  },
  {
    id: 51,
    question: "Як використовувати списки та карти (maps) у SCSS?",
    answer:
      "Список — це впорядкований набір значень (через пробіл або кому, із/без дужок). Карта — це ключ → значення (як об’єкт/словник).",
  },
  {
    id: 52,
    question:
